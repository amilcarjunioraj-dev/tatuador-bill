export default function Tela7_Agendamento({ irParaTela, dados, atualizarDados }) {
  ...
  const verificarHorario = (hora) => {
    ...
    const fimEstimado = horas + Math.ceil(tempo / 60);
    return fimEstimado <= 18;
  };
          ...
          <div key={index} style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              {horariosDisponiveis.map((hora) => (
                <div
                  key={hora}
                  className={`horario-card ${dados.hora === hora && formatarData(dados.data_obj) === formatarData(data) ? 'selecionado' : ''}`}
